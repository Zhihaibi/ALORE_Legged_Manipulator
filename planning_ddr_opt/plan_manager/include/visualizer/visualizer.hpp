...
class Visualizer
{
  ...
  public:
    ...
    ros::Publisher obstacle_area_pub_; // New publisher for obstacle areas
    ...
    Visualizer(ros::NodeHandle nh){
      ...
      obstacle_area_pub_ = nh_.advertise<visualization_msgs::Marker>("/planner/obstacle_areas", 10);
      
    }

    ~Visualizer(){};

    void publishObstacleAreas(const std::vector<Eigen::Vector3d>& item_positions, double area_size) {
        visualization_msgs::Marker cube_list;
        cube_list.header.frame_id = "world";
        cube_list.header.stamp = ros::Time::now();
        cube_list.ns = "obstacle_areas";
        cube_list.id = 0;
        ...
        cube_list.scale.x = area_size;
        cube_list.scale.y = area_size;
        cube_list.scale.z = 0.1; // A small height for visualization
        cube_list.color.r = 1.0;
        cube_list.color.g = 0.0;
        cube_list.color.b = 0.0;
        cube_list.color.a = 0.5;

        for (const auto& pose : item_positions) {
            geometry_msgs::Point p;
            p.x = pose.x();
            p.y = pose.y();
            p.z = 0.05; // Center the cube at a small height
            cube_list.points.push_back(p);
        }

        obstacle_area_pub_.publish(cube_list);
    }
